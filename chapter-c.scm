...
;;;; C.5.2 探索の手法 (p539)
;; データ以下のとおり木構造で定義し、探索対象とする
(define data '((a1 (b1) (b2)) (a2 (b3))))

;; データを一時退避させる領域が stack である場合、深さ優先探索となり、
;; queue である場合、幅優先探索となる。
;; まずは stack で管理する深さ優先探索 (dfs) (p540) について考える。
;; まず、data を stack に登録する。
(set-stack)
(push data)

(define search (lambda (name tree)
 (define path (lambda (insert delete dig)
   (let loop ((tmp '()))
     (let ((w (delete)))
       (if (null? w)
           (reverse tmp)
...
(search "dfs" data)
;; out (a1 b1 b2 a2 b3 a1 b1 b2 a2 b3)
;; あれ？出力に重複がある。何かを写し間違えたようだ
